...
public class BOOTP {
	...
	public BOOTP(int xid, byte[] yiaddr, byte[] server_ip, byte[] bootp_gw_addr, byte[] hw_addr, byte[] server_name, byte[] boot_file) {
		this.xid = xid; 
		this.yiaddr = yiaddr;
		this.server_ip = server_ip;
		this.bootp_gw_addr = bootp_gw_addr;
		
		Arrays.fill(this.hw_addr, (byte) 0);
		for (int i=0; i<hw_addr.length; i++)
			this.hw_addr[i] = hw_addr[i];
		
		Arrays.fill(this.server_name, (byte) 0);
		for (int i=0; i<server_name.length;i++)
			this.server_name[i] = server_name[i];
		
		Arrays.fill(this.boot_file, (byte)0);
		for (int i=0; i<boot_file.length; i++)
			this.boot_file[i] = boot_file[i];
	}
